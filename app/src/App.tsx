...
import { solveChallenge, verifyProof } from './lib/pow';
import './App.css';
              ...
              <form
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    const proof = await solveChallenge(formFingerprint);
                    const isValid = await verifyProof(proof);
                    if (!isValid) {
                      setFormError('Verification couldn\'t be completed. Please refresh the page and try again.');
                      setFormStatus('error');
                      return;
                    }
                    ...
                    const { data, error } = await supabase.functions.invoke('submit-inquiry', {
                      body: { name, email, phone, project, proof }
                    });

                    if (error || (data && data.error)) {
                      setFormError(error?.message || data?.error || 'Your message couldn\'t be sent right now. Please try again in a moment.');
                      setFormStatus('error');
                      return;
                    }
                    ...
                    setFormError('');
                    localStorage.setItem('mace_last_submit', String(now));
                    setPhoneNumber('');
                    form.reset();
                    setTimeout(() => setFormStatus('idle'), 5000);
                  } catch (err: any) {
                    setFormError(err?.message || 'Something went wrong. Please try again later.');
                    setFormStatus('error');
                  }
                }}
              >
